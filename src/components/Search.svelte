<script lang="ts">
  ...
  import { Icon, MusicalNote, Microphone } from "svelte-hero-icons";
...
{#if songQuery != null && artistQuery != null}
  <div class="relative z-10">
    <div
      class="shadow border rounded absolute top-0 left-4 right-4 bg-white max-w-lg"
    >
      {#await songQuery then songs}
        <div>
          <h3 class="text-md font-bold my-4 mx-8">သီချင်းများ</h3>
      ...
      {#await artistQuery then artists}
        <div>
          <h3 class="text-md font-bold my-4 mx-8">အဆိုတော်များ</h3>
